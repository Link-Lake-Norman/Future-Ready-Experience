...
import { facilitators } from "@/content/admin-portal";

export default function Page() {
  const [query, setQuery] = useState("");
  const filtered = useMemo(() => facilitators.filter((item) => JSON.stringify(item).toLowerCase().includes(query.toLowerCase())), [query]);

  return (
    <AdminShell activePath="/admin/facilitators">
      <AdminHero
        eyebrow="FACILITATOR MANAGEMENT"
        title="Manage facilitators, training status, cohorts, and student assignments."
      ...
      <section style={{ ...adminStyles.card, marginTop: "22px" }}>
        <div style={adminStyles.padding}>
          <div style={adminStyles.eyebrow}>FACILITATORS DIRECTORY</div>
          <h2 style={adminStyles.title}>Facilitators</h2>
        </div>
        ...
        <div style={adminStyles.toolbar}>
          ...
          />
          <button style={adminStyles.button}>Add Facilitator</button>
        </div>
