...
import { adminStudents } from "@/content/admin-portal";

export default function Page() {
  const [query, setQuery] = useState("");
  const filtered = useMemo(() => adminStudents.filter((item) => JSON.stringify(item).toLowerCase().includes(query.toLowerCase())), [query]);

  return (
    <AdminShell activePath="/admin/students">
      <AdminHero
        eyebrow="STUDENT MANAGEMENT"
        title="Review student enrollment, readiness, attendance, and cohort assignment."
      ...
      <section style={{ ...adminStyles.card, marginTop: "22px" }}>
        <div style={adminStyles.padding}>
          <div style={adminStyles.eyebrow}>STUDENTS DIRECTORY</div>
          <h2 style={adminStyles.title}>Students</h2>
        </div>
        ...
        <div style={adminStyles.toolbar}>
          ...
          />
          <button style={adminStyles.button}>Add Student</button>
        </div>
