...
import { organizations } from "@/content/admin-portal";

export default function Page() {
  const [query, setQuery] = useState("");
  const filtered = useMemo(() => organizations.filter((item) => JSON.stringify(item).toLowerCase().includes(query.toLowerCase())), [query]);

  return (
    <AdminShell activePath="/admin/organizations">
      <AdminHero
        eyebrow="ORGANIZATION MANAGEMENT"
        title="Manage schools, employers, and community partners."
      ...
      <section style={{ ...adminStyles.card, marginTop: "22px" }}>
        <div style={adminStyles.padding}>
          <div style={adminStyles.eyebrow}>ORGANIZATIONS DIRECTORY</div>
          <h2 style={adminStyles.title}>Organizations</h2>
        </div>
        ...
        <div style={adminStyles.toolbar}>
          ...
          />
          <button style={adminStyles.button}>Add Organization</button>
        </div>
