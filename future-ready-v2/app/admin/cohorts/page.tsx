...
import { cohorts } from "@/content/admin-portal";

export default function Page() {
  const [query, setQuery] = useState("");
  const filtered = useMemo(() => cohorts.filter((item) => JSON.stringify(item).toLowerCase().includes(query.toLowerCase())), [query]);

  return (
    <AdminShell activePath="/admin/cohorts">
      <AdminHero
        eyebrow="COHORT MANAGEMENT"
        title="Manage program delivery by cohort, schedule, facilitator, and enrollment."
      ...
      <section style={{ ...adminStyles.card, marginTop: "22px" }}>
        <div style={adminStyles.padding}>
          <div style={adminStyles.eyebrow}>COHORTS DIRECTORY</div>
          <h2 style={adminStyles.title}>Cohorts</h2>
        </div>
        ...
        <div style={adminStyles.toolbar}>
          ...
          />
          <button style={adminStyles.button}>Add Cohort</button>
        </div>
