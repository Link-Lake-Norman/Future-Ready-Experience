...
import { programs } from "@/content/admin-portal";

export default function Page() {
  const [query, setQuery] = useState("");
  const filtered = useMemo(() => programs.filter((item) => JSON.stringify(item).toLowerCase().includes(query.toLowerCase())), [query]);

  return (
    <AdminShell activePath="/admin/programs">
      <AdminHero
        eyebrow="PROGRAM MANAGEMENT"
        title="Manage curriculum models, delivery structures, and active implementations."
      ...
      <section style={{ ...adminStyles.card, marginTop: "22px" }}>
        <div style={adminStyles.padding}>
          <div style={adminStyles.eyebrow}>PROGRAMS DIRECTORY</div>
          <h2 style={adminStyles.title}>Programs</h2>
        </div>
        ...
        <div style={adminStyles.toolbar}>
          ...
          />
          <button style={adminStyles.button}>Add Program</button>
        </div>
